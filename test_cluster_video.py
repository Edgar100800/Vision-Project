#!/usr/bin/env python3
"""
Video Clustering Analysis Script
===============================
This script processes a video file directly to perform person re-identification
clustering analysis using the optimized three_person_dataset_90_confidence preset.
"""
...
def extract_person_crops_from_video(video_path, confidence_threshold=0.9, frame_skip=1, max_frames=None):
    ...
    Returns:
        List of (crop_image, frame_number, detection_confidence, bbox)
    """
...
def perform_clustering_analysis(features, crop_metadata, clustering_cfg, reid_cfg):
    """Perform clustering analysis on the extracted features."""
    print(f"\n🔍 Performing clustering analysis...")
    print(f"   📋 Using threshold: {clustering_cfg['mahalanobis_threshold']}")
    ...
    cluster_mgr = GaussianClusterManager(
        ...
        mahalanobis_threshold=clustering_cfg['mahalanobis_threshold'],
        max_clusters=clustering_cfg['max_clusters'],
        min_samples_for_update=clustering_cfg['min_samples_per_cluster'],
        fusion_threshold=clustering_cfg['mahalanobis_threshold'] * 0.8,
        split_threshold=clustering_cfg['mahalanobis_threshold'] * 2.0,
...
def generate_visualizations(clustering_results, output_dir):
    ...
    ax1.set_title(f'Cluster Assignments ({len(unique_clusters)} clusters)')
    ...
    ax2.set_title('Temporal Distribution of Clusters')
    ...
    # Save visualization
    viz_path = output_dir / 'video_clustering_analysis.png'
    plt.savefig(viz_path, dpi=300, bbox_inches='tight')
    plt.close()

    print(f"   ✅ Visualization saved to: {viz_path}")

    # Create additional timeline visualization
...
def main(video_path: str, output_dir: str, frame_skip: int = 1, max_frames: int = None):
    """Main function to process video and perform clustering analysis."""
    ...
    print("="*80)
    print("🎬 VIDEO CLUSTERING ANALYSIS")
    print("="*80)
    ...
    viz_path, timeline_path = generate_visualizations(
    ...
    results = {
        ...
        'configuration': {
            ...
            'mahalanobis_threshold': float(clustering_cfg['mahalanobis_threshold']),
            'expected_persons': int(clustering_cfg['expected_persons'])
        }
    }

    results_path = output_dir / 'video_clustering_results.json'
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Video clustering analysis')
    parser.add_argument('--video', default='data/raw/video2.mp4',
                        help='Path to video file')
    parser.add_argument('--output-dir', default='output/video_clustering',
