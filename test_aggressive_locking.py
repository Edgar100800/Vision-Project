#!/usr/bin/env python3
"""
Aggressive ID locking test - prevents ID swaps after consistent tracking.
"""
...
def test_aggressive_locking():
    """Test tracking with aggressive ID locking to prevent swaps."""
    ...
    # Setup logging
    setup_logger("INFO", "logs/aggressive_locking.log")
    
    # Load configuration with more aggressive settings
    config = ConfigLoader("config/config.yaml")
    
    # Override config for more aggressive locking
    config.get('tracking')['reid_threshold'] = 0.8  # Higher threshold
    config.get('tracking')['history_threshold'] = 0.75  # Higher threshold
    config.get('tracking')['spatial_threshold'] = 80  # Lower spatial threshold
    config.get('tracking')['min_track_frames'] = 8  # More frames before re-identification
    config.get('tracking')['id_lock_threshold'] = 0.85  # Higher lock threshold
    config.get('tracking')['id_lock_frames'] = 5  # More frames before locking
    ...
    output_path = "data/processed/output_tracking_aggressive.mp4"
    ...
    print(f"Aggressive locking test on {total_frames} frames at {fps} FPS")
    print(f"Video size: {width}x{height}")
    print("=" * 50)
    
    # Setup video writer with compatible codec for Windows
    try:
        ...
        print(f"✅ VideoWriter inicializado con H264: {output_path}")
    except:
        try:
            # Try XVID codec as fallback
            output_path_xvid = output_path.replace('.mp4', '.avi')
            fourcc = cv2.VideoWriter_fourcc(*'XVID')
            out = cv2.VideoWriter(output_path_xvid, fourcc, fps, (width, height))
            if not out.isOpened():
                raise Exception("XVID not available")
            print(f"✅ VideoWriter inicializado con XVID: {output_path_xvid}")
            output_path = output_path_xvid  # Update path for later use
        except:
            # Last resort: try MJPG
            output_path_mjpg = output_path.replace('.mp4', '.avi')
            fourcc = cv2.VideoWriter_fourcc(*'MJPG')
            out = cv2.VideoWriter(output_path_mjpg, fourcc, fps, (width, height))
            if not out.isOpened():
                print(f"❌ Error: No se pudo abrir el VideoWriter con ningún codec disponible")
                print(f"   Tried: H264, XVID, MJPG")
                return
            print(f"✅ VideoWriter inicializado con MJPG: {output_path_mjpg}")
            output_path = output_path_mjpg  # Update path for later use
    
    # Verify video writer is properly initialized
    if not out.isOpened():
        print(f"❌ Error: No se pudo abrir el VideoWriter para {output_path}")
        print(f"   Codec: mp4v, FPS: {fps}, Size: {width}x{height}")
        return
    else:
        print(f"✅ VideoWriter inicializado correctamente: {output_path}")
    
    # Track frames written
    frames_written = 0
    
    # Track ID consistency and locking
    id_changes = 0
    ...
    locked_ids = set()  # Track locked IDs
    ...
    while True:
        ...
        # Check for newly locked IDs
        locked_info = tracker.get_locked_ids_info()
        for person_id in locked_info['locked_persons']:
            if person_id not in locked_ids:
                locked_ids.add(person_id)
                print(f"🔒 ID LOCKED at frame {frame_count}: Person {person_id}")
        ...
        if frame_count % 50 == 0:
            print(f"📹 Frame {frame_count}: {frames_written} frames escritos al video")
        
        # Show frame (optional - comment out if you don't want to see it)
        cv2.imshow('Aggressive Locking Test', visualized_frame)
        ...
        if frame_count % 25 == 0:
            ...
            for track in tracking_result.tracks:
                if track.is_active:
                    is_locked = track.person_id in locked_info['locked_persons']
                    lock_status = "🔒" if is_locked else "🔓"
                    track_length = getattr(track, 'get_track_length', lambda: "N/A")()
                    print(f"    Track {track.track_id} -> Person {track.person_id} {lock_status} (frames: {track_length})")
            
            # Print used person IDs
            print(f"    Used person IDs: {sorted(used_person_ids)}")
            print(f"    New persons detected: {new_person_counter}")
            print(f"    Locked IDs: {sorted(locked_ids)}")
    ...
    print("\n" + "=" * 50)
    print("FINAL RESULTS - AGGRESSIVE LOCKING")
    print("=" * 50)
    ...
    if video_file.exists():
        ...
        if file_size == 0:
            ...
            print("⚠️  Warning: Video file is very small, may be corrupted!")
        else:
            print(f"✅ Video file size looks good: {file_size} bytes")
    else:
    ...
    if len(locked_ids) > 0:
        print(f"✅ {len(locked_ids)} IDs were locked to prevent swaps!")
    else:
        print("⚠️  No IDs were locked - may need more aggressive settings")
    ...
    print(f"\n🎬 Video processing complete. Check: {output_path}")


if __name__ == "__main__":
    test_aggressive_locking() 